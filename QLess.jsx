...
// ── ADMIN PAGE ────────────────────────────────────────────────────────────────
function AdminPage({ onLogout, menu, setMenu, liveOrders, setLiveOrders }) {
            ...
            {activeOrders.length === 0 ? (
              ...
              <div className="orders-grid">
                {activeOrders.map(order => (
                  <div key={order.id} className={`live-order-card ${order.status === "preparing" ? "new" : ""}`}>
                    ...
                    <div className="live-order-footer">
                      ...
                      <div className="status-btns">
                        {order.status === "preparing" && (
                          <button className="status-btn ready" onClick={() => updateStatus(order.id, "ready")}>Mark Ready</button>
                        )}
                        {order.status === "ready" && (
                          <button className="status-btn completed" onClick={() => updateStatus(order.id, "completed")}>Complete</button>
                        )}
                      </div>
                    </div>
                  </div>
